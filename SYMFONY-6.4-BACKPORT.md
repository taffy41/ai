...
```bash
git fetch upstream
git log --oneline 5fca0037..upstream/main | wc -l   # how many new commits (informational)
...
```bash
grep -rnE 'stringNode|enumFqcn|scalarNodeFqcn' src --include='*.php'   # expect: no output
```
...
feature. As of the last sync these files were affected and were rewritten to classic
`Command` subclasses (`configure()` + `execute()`):
...
cd src/platform && vendor/bin/phpunit && cd -
cd src/ai-bundle && vendor/bin/phpunit && cd -
```
...
> at `^0.9` (still 7.x). Use the monorepo CI's path-repository setup, or the `./link`
...
## 6. Reference: composer constraint fixer script
